...
**When a test disagrees with the code, work out which is wrong before changing either.** Thirty-two defects were found this way. Every one passed review, and most passed unit tests too. They surfaced only when the code met something real: a live site, a real keyword expansion, a real command line, a packaged install driven over stdio by a client that had never seen the source tree (9 and 10), the documented workflow executed in order with each tool fed the previous one output (11 and 12), or a census that called all 36 tools and bucketed the results by error code (13). Number 14 surfaced because grep kept returning nothing for a file that plainly contained the string. Number 25 surfaced from asking which tools a *specific* free provider actually covers, rather than which capability it switches on. Number 26 surfaced from running a CI step by hand instead of trusting that it had ever passed. Numbers 27-32 all surfaced within twenty minutes of the first real API keys arriving, on paths that 92 fixture tests had pronounced healthy. Reviewing the repo will not find these.
...
26. The CI step named "Verify the packed tarball is runnable" did not verify that. It ran `npm pack`, extracted the tarball, and invoked `node dist/cli.js` inside the extract — but the tarball ships `dist` and no `node_modules`, and nothing installs the dependencies, so the command dies on `ERR_MODULE_NOT_FOUND: zod` before reaching a single line of this codebase. The step had never run on a hosted runner, so it had never had the chance to fail. It now installs the tarball into an empty `npm init` project, asserts both bin links exist, and runs through `node_modules/.bin/seo-agent` — which is what a user actually does, and covers the `bin` mapping that the old form skipped entirely. **A packaging check that never installs is not a packaging check**, and a build step that has never executed is an untested assertion no matter how sensible it reads.

27. `SEO_AGENT_BUDGET` caps *money* — its own doc comment says so, citing the credit card. But `reserve()` was called with a non-zero cost for two providers whose keys are **free**: PageSpeed Insights and the Open PageRank free tier. So `SEO_AGENT_BUDGET=0`, which the README recommends as the way to stay free, disabled two of the three free-credential tools it had just told the user to configure. The docs contradicted themselves and the code sided with the wrong one. Cost is now a property of billability: free providers carry cost `0`, so `reserve(0)` passes at a cap of 0 while `record()` still logs the call and `seo_usage` still reports the count. Genuinely paid tools remain blocked, verified. The general shape: a "spend" guard and a "quota" guard are different mechanisms, and a cap that conflates them will always misfire on the free tier first.

28. The PageSpeed opportunity parser read savings only from `details.overallSavingsMs`. Live v5 responses also carry the newer `*-insight` audits, which report savings in a **top-level `metricSavings`** object instead — so every insight audit parsed to `savings_ms: null`, the whole list tied at zero, and the sort-by-savings that ordered it was inert. Measured live: wikipedia.org's `cache-insight` is worth 250ms of LCP and was reported as unknown; on bbc.com/news the correct ordering puts `unused-javascript` (2750ms, classic shape) above `cache-insight` (1950ms, insight shape) — two shapes that must be compared against each other, not parsed separately. `savingsMs()` now reads both and takes the largest single metric, since per-metric savings overlap and summing double-counts. CLS is excluded: it is a unitless layout score, not milliseconds.

29. The same parser selected opportunities by `scoreDisplayMode === 'numeric'`, which admits pure diagnostics. `max-potential-fid` — a measurement of the longest task, with no remedy attached — was being handed to agents as an optimisation opportunity. Selection is now by actionability: classic `details.type === 'opportunity'`, or an audit carrying `metricSavings`. **A metric is not a fix**, and a list called `opportunities` must contain only things that can be applied.

30. `seo_page_speed` returned **zero actions** for bbc.com/news: 31/100 lab score, ~8.5s of recoverable load time, and a summary reading "Passes the Core Web Vitals assessment". The only action branch fired on a *failing* Core Web Vital, so a page that passes on field data reported nothing to do regardless of how much time was recoverable — leaving the agent to re-derive priority from `data.opportunities`, which is the second reasoning pass this codebase exists to remove. A low-priority headroom action now fires when a passing page still has ≥1s recoverable or a lab score under 50, capped in impact so it can never outrank a genuine failure in the shared backlog. Passing a threshold is not the same as having nothing to do.

31. The CLI silently ignored unknown flags. zod objects strip unknown keys, so `--dimension date` (the field is `dimensions`) was dropped and the tool ran with its default of `["query"]` — returning a confident, plausible, correctly-formed answer to a *different question* than the one asked, with `ok: true` and no warning. Near-miss flag names are the easiest mistake to make and were the only one that failed invisibly. Unknown arguments are now rejected before execution, with a did-you-mean for near matches and the valid list. This is defect 7's twin: both were CLI-only, because the MCP path receives typed JSON validated against the advertised schema.

32. `seo_gsc_performance` returned `ok: true`, "0 clicks, 0 impressions ... across 0 query row(s)" and **no warning** for a window that happened to be empty. Measured on a real property: the last 28 days returned nothing while a 480-day window held 589 impressions across 19 queries. Bare, that output reads as "this site ranks for nothing" — a confident false negative on the exact question the tool exists to answer, and the same failure as defect 15 in a different place. An empty result now explains itself, and distinguishes "no rows at all" from "rows existed but `min_impressions` excluded them", since those have opposite remedies.
...
**Some strings cannot be reached at runtime.** The two `seo_gsc_list_sites` typos lived on a credential-gated path. Search Console and PageSpeed have since been exercised live and both had real defects (27-32); the remaining paid providers' live HTTP paths are still unexercised because there are no keys to exercise them with. `tests/references.test.ts` covers that gap the only way available: a static grep over `src/` asserting every tool name mentioned anywhere resolves. It needs no credentials and no network. When a check cannot be a runtime test, make it a static one rather than leaving it unmade.
...
Verified at the time of writing: 446 tests, 53 rules in 8 categories, 36 tools, 13 credential-gated, and the reference health scores — stripe.com 18.2, vercel.com 42, iana.org 19.3, example.com 71 at `max_pages: 8`. Those four were re-measured after the duplicate-eligibility floor changed, since that could have shifted them; they held exactly.
